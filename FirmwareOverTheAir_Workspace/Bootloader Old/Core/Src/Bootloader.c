...
BL_Status BL_Fetch_Commend(void) {
	...
	/* To clear buffer of RX and prevent garbage messages of buffer */
...
#endif
	/* Host command format :
	   => Command Length  (1 byte = Data_Length )
	 */
	...
	if (HAL_Status != HAL_OK){
		...
		/*
		 Depending on Data_Length we will receive the number of bytes of the sending code
		 Commend Code (1 byte) + Details (Data_Length) + CRC (4 bytes)
		 Where :
		 => Command Code is the order that Host want to do in code
		 => Details explain what you transmit
		 => CRC is safety algorithm on code
		 */
		Data_Length = BL_HostBuffer[0];

		/* we determine the number of receiving bytes next from the first number transmit in first
		...
		if (HAL_Status != HAL_OK){
			...
			if (BL_HostBuffer[1]>=FIRST_COMMEND &&BL_HostBuffer[1]<=LAST_COMMEND ){ ///////// Let LAST_COMMEND = 0x17
				Bootloader_Functions[BL_HostBuffer[1]-FIRST_COMMEND](BL_HostBuffer);
			}
			else {
#if BL_DEBUG_ENABLE == DEBUG_INFO_ENABLE
				BL_PrintMassage ("%d\n",BL_HostBuffer[1]);
				BL_PrintMassage ("Invalid commend code received from host !! \r\n");
#endif
				Status = BL_NACK ;
			}
		}
...
static void Bootloader_Send_Data_To_Host(uint8_t *Host_Buffer , uint32_t Data_Len){
	HAL_UART_Transmit(BL_HOST_COMMUNICATION_UART,(uint8_t*) Host_Buffer,(uint16_t) Data_Len, HAL_MAX_DELAY); ///////////////////////May cause bug
}
...
static void Bootloader_Get_Version (uint8_t *Host_Buffer){
	...
	if ( CRC_OK == Bootloader_CRC_Verify(Host_Buffer, (Host_CMD_Packet_Len - CRC_TYPE_SIZE), Host_CRC32)){
		...
		Bootloader_Send_Data_To_Host(BL_Version, VERSION_LENGTH);
...
#endif
			Bootloader_Send_Data_To_Host((uint8_t *)&Address_Verification_State, ADDRESS_BL_ACK_LENGTH);
...
static uint8_t Perform_Flash_Erase (uint32_t PageAddress, uint8_t Number_Of_Pages){
	...
	/* erase all memory or specific page */
		...
		if ( CBL_FLASH_MASS_ERASE == PageAddress  ){ //////////////////////////////////////////////////////////////////////////Change this when using Two Apps
			pEraseInit.PageAddress = FLASH_PAGE_BASE_ADDRESS_APP;
			pEraseInit.NbPages = APPLICATION_SIZE;
#if BL_DEBUG_ENABLE == DEBUG_INFO_ENABLE
			BL_PrintMassage ("Mass erase \r\n");
#endif
		}
		/* erase specific page */
		else {
...
static void Bootloader_Erase_Flash (uint8_t *Host_Buffer){
	...
	if ( CRC_OK == Bootloader_CRC_Verify(Host_Buffer, (Host_CMD_Packet_Len - CRC_TYPE_SIZE), Host_CRC32)){
		...
		Erase_Status = Perform_Flash_Erase ( *( (uint32_t*)&Host_Buffer[2] ),Host_Buffer[6]); //Host_Buffer[6] contains number of pages
		                                                                                      //Host_Buffer[2-->5] contains base address of APP
		/* Report the erase state */
		Bootloader_Send_Data_To_Host((uint8_t *)(&Erase_Status), ERASE_BL_ACK_LENGTH);
		if ( SUCESSFUL_ERASE == Erase_Status){
#if BL_DEBUG_ENABLE == DEBUG_INFO_ENABLE
			BL_PrintMassage("Successful erased\r\n");
#endif
		}
		else {
#if BL_DEBUG_ENABLE == DEBUG_INFO_ENABLE
			BL_PrintMassage("Unsuccessful erased\r\n");
#endif
		}
#if PYTHON == 1
	}
	...
	if (HAL_Status == HAL_OK) {
		...
		while (Payload_Len <=0 && Status == HAL_OK ){
			Payload_Buffer = (uint16_t) Host_PayLoad[PayLoad_Counter]
													 |( (uint16_t)Host_PayLoad[PayLoad_Counter+1] << TWO_BYTES );//?????????????????????????????????????????????????????????????????????????????????????????????What is that mean

			/*update the flash address each iteration */
...
static void Bootloader_Memory_Write (uint8_t *Host_Buffer){
	...
	if(CRC_OK == Bootloader_CRC_Verify(Host_Buffer,(Host_CMD_Packet_Len - CRC_TYPE_SIZE), Host_CRC32))
	{
		...
		if(ADDRESS_IS_VALID == Address_Verification)
		{
			...
			Bootloader_Send_Data_To_Host((uint8_t *)&Flash_Payload_Write_Status, WRITE_PAYLOAD_BL_ACK_LENGTH);
		}
		else
		{
#if BL_DEBUG_ENABLE == DEBUG_INFO_ENABLE
			...
			Bootloader_Send_Data_To_Host((uint8_t *)&Address_Verification, WRITE_PAYLOAD_BL_ACK_LENGTH);
		}
#if PYTHON == 1
	}
	else
...
void BL_PrintMassage(char *format, ...) {
	...
	HAL_UART_Transmit(BL_DEBUG_UART, (uint8_t*) Message, (uint16_t)sizeof(Message),
			HAL_MAX_DELAY);///////////////////////////////////////////////////////////////HAL_MAX_DELAY May cause bug
