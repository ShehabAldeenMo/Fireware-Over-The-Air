...
void Get_Erase_Config(void)
{
	...
	{ 
		client.publish("Ali_FromEspToBroker","Please Enter Page ID to Erase");
	...
	else if((MQTT_Message >= PAGE_ID_LOWER_LIMIT) && (MQTT_Message <= PAGE_UPPER_LIMIT) && (Global_Erase_Counter == 1))
	{
		...
		client.publish("Ali_FromEspToBroker","Please Enter Page Amount to Erase");		
...
void Ack_Erase(void)
{
	...
	if(SUCESSFUL_ERASE  == Ack)
	{
		...
		/*Node-Red Ack*/
		client.publish("Ali_FromEspToBroker","Erase operation successfuly completed!!");
	}
	else
	{
		/*Serial monitor Nack*/
		...
		/*Node-Red Nack*/
		client.publish("Ali_FromEspToBroker","Erase operation Failed!!, Please try again");
	}
...
void Erase_Memory(void)
{
	...
	if(Global_Erase_Counter == 4) //When Global_Erase_Counter = 3, This means all configrations are made.
	{
		...
		digitalWrite(led,HIGH);//For debug
		client.publish("Ali_FromEspToBroker","Erasing From ECU..");
...
void Jump_To_App(void)
{
	...
	digitalWrite(led,HIGH);//For debug
	client.publish("Ali_FromEspToBroker","Jumping to App..");
...
void Jump_To_BL(void)
{
  /*Notify user beginning of Jumping to App process */
	digitalWrite(led,HIGH);//For debug
	client.publish("Ali_FromEspToBroker","Jumping to Bootloader..");
  ...
  {
    client.publish("Ali_FromEspToBroker", "Receive Ack from BL jump function in STM");
  }
  else if(Ack == BL_BEGIN_NOT_MATCH)
  {
    client.publish("Ali_FromEspToBroker", "Receive Nack from BL jump function in STM!!");
  }
  else
  {
    client.publish("Ali_FromEspToBroker", "Undefined behaviour from BL jump function in STM!!");
  }
